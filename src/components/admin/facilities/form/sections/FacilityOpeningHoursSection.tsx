
import React, { forwardRef, useImperativeHandle } from 'react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Clock, Plus } from 'lucide-react';
import { useOpeningHours, useOpeningHoursMutations } from '@/hooks/useOpeningHours';

interface FacilityOpeningHoursSectionProps {
  facilityId?: number;
}

export const FacilityOpeningHoursSection = forwardRef<
  { saveData: () => Promise<boolean> },
  FacilityOpeningHoursSectionProps
>(({ facilityId }, ref) => {
  const { data: openingHours = [] } = useOpeningHours(facilityId || 0);
  const { saveOpeningHours } = useOpeningHoursMutations();

  useImperativeHandle(ref, () => ({
    saveData: async () => {
      try {
        if (facilityId) {
          await saveOpeningHours({ facilityId, hours: openingHours });
        }
        return true;
      } catch (error) {
        console.error('Error saving opening hours:', error);
        return false;
      }
    }
  }));

  const handleAddHours = () => {
    console.log('Add opening hours for facility:', facilityId);
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Clock className="h-5 w-5" />
            Opening Hours
          </div>
          <Button onClick={handleAddHours} size="sm">
            <Plus className="h-4 w-4 mr-2" />
            Add Hours
          </Button>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {!facilityId ? (
          <p className="text-gray-500">Save the facility first to manage opening hours</p>
        ) : openingHours.length === 0 ? (
          <div className="text-center py-8">
            <Clock className="h-16 w-16 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-500 mb-2">No opening hours configured</p>
            <p className="text-sm text-gray-400">
              Add opening hours to define when the facility is available
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {openingHours.map((hours, index) => (
              <div key={index} className="p-3 border rounded-lg">
                Opening hours configuration
              </div>
            ))}
          </div>
        )}
      </CardContent>
    </Card>
  );
});

FacilityOpeningHoursSection.displayName = 'FacilityOpeningHoursSection';
