...
const FacilityListItem: React.FC<FacilityListItemProps> = ({
  ...
  return <Card className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-0 shadow-md hover:shadow-xl transform hover:-translate-y-1 bg-white">
        ...
        <div className="flex h-48 sm:h-56 md:h-64">
          ...
          <div className="w-48 sm:w-64 md:w-72 lg:w-80 flex-shrink-0">
            <div className="w-full h-full rounded-l-lg overflow-hidden">
              <FacilityListItemImage facilityId={facility.id} facilityName={facility.name} facilityType={facility.type} area={facility.area} />
            </div>
          </div>
          ...
          <div className="flex-1 p-6 sm:p-8 flex flex-col justify-between min-w-0 mx-4">
            <div className="flex-1">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center text-gray-600 mt-2">
                    ...
                    <span className="text-base truncate hover:text-blue-600 cursor-pointer" onClick={handleAddressClick}>
                      {facility.address}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-3 ml-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <Button variant="ghost" size="sm" className="h-10 w-10 p-0 hover:bg-gray-100" onClick={handleFavorite}>
                    <Heart className={`h-5 w-5 ${isFavorited ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-10 w-10 p-0 hover:bg-gray-100" onClick={handleShare}>
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-base text-gray-600 mb-4">
                ...
                {facility.rating && <div className="flex items-center">
              ...
              {/* Description */}
              {facility.description && <p className="text-gray-600 text-base line-clamp-2 mb-4">
                  {facility.description}
                </p>}

              {/* Equipment/Features */}
              {facility.equipment && facility.equipment.length > 0 && <div className="flex flex-wrap gap-2 mb-4">
                  {facility.equipment.slice(0, 3).map((item, index) => <Badge key={index} variant="secondary" className="text-sm px-3 py-1">
                      {item}
                    </Badge>)}
                  {facility.equipment.length > 3 && <Badge variant="outline" className="text-sm px-3 py-1">
          ...
          <div className="w-48 sm:w-64 md:w-72 lg:w-80 flex-shrink-0">
            <div className="w-full h-full rounded-r-lg overflow-hidden">
              <FacilityListItemMap address={facility.address} facilityName={facility.name} />
            </div>
          </div>
        </div>
      </CardContent>
