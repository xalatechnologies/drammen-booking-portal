...
export function CalendarGrid({ 
  ...
  const renderTimeSlotCell = (day: Date, timeSlot: string, dayIndex: number) => {
    ...
    const cell = (
      <button
        className={`w-full h-8 rounded border transition-all duration-200 text-xs font-medium ${statusStyle} ${
      ...
      >
        <div className="flex items-center justify-center h-full">
          <span className={`text-xs ${isSelected ? 'text-white' : 'text-gray-700'}`}>
            {startTime}
          </span>
          {isSelected && (
