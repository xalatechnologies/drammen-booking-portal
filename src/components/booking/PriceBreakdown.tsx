...
import { Calculator, Tag, TrendingUp, TrendingDown } from 'lucide-react';
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-white border border-blue-200 rounded-lg p-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-2xl font-bold text-blue-600">
              {calculation.finalPrice.toFixed(2)} kr
            </span>
          </div>
        ...
        {showDetailed && calculation.breakdown.length > 0 && (
          <div className="space-y-2">
            <h4 className="font-medium text-blue-900 text-sm">Prisoppbygging:</h4>
            <div className="bg-white border border-blue-200 rounded-lg overflow-hidden">
              {calculation.breakdown.map((item, index) => (
                <div
                  key={index}
                  className={`flex justify-between items-center p-3 ${
                    index !== calculation.breakdown.length - 1 ? 'border-b border-blue-100' : ''
                  }`}
                >
                  <div className="flex items-center gap-2">
