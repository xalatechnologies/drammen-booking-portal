...
export const FacilityBlackoutService = {
  getBlackouts: async (pagination?: PaginationParams, filters?: any) => {
    ...
    const result = await facilityBlackoutRepository.getAll(pagination, filters);
  ...
  getBlackoutById: async (id: string) => {
    ...
    const result = await facilityBlackoutRepository.getById(id);
  ...
  createBlackout: async (blackoutData: any) => {
    ...
    return await facilityBlackoutRepository.createAsync(blackoutData);
  },
  ...
  updateBlackout: async (id: string, blackoutData: any) => {
    ...
    return await facilityBlackoutRepository.updateAsync(id, blackoutData);
  },
  
  deleteBlackout: async (id: string) => {
    console.log('FacilityBlackoutService.deleteBlackout - Called with ID:', id);
    return await facilityBlackoutRepository.deleteAsync(id);
  },
  ...
  getBlackoutsByFacility: async (facilityId: number) => {
    ...
    return await facilityBlackoutRepository.getAll({ page: 1, limit: 100 }, { facilityId });
  }
};
