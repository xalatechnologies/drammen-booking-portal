...
const FacilityImage: React.FC<FacilityImageProps> = ({ image, name, type }) => {
  return (
    <div className="w-80 h-full bg-gradient-to-br from-gray-100 to-gray-200 relative overflow-hidden flex-shrink-0 rounded-l-lg">
      <img 
        src={image} 
        alt={name} 
      ...
      <div className="absolute top-4 right-4">
        <Badge className="bg-white/90 backdrop-blur-sm text-slate-700 border-0 font-semibold text-sm px-3 py-1.5 shadow-sm">
          {type}
        </Badge>
      </div>
    </div>
